...
import { useListProgramStreamsQuery, type ProgramStreamRecord } from "@/services/programstream/programStreamApi";

const PAGE_SIZE = 10;
const SORT_DESCENDING = -1;

type ProgramStreamTableProps = {
  onEdit: (record: ProgramStreamRecord) => void;
  onDelete: (record: ProgramStreamRecord) => void;
};

export function ProgramStreamTable({ onEdit, onDelete }: ProgramStreamTableProps) {
  const [pageNumber, setPageNumber] = useState(1);

  const { data, isLoading, isFetching, isError, error } = useListProgramStreamsQuery({
    pageNumber,
    pageSize: PAGE_SIZE,
            ...
            {isLoading ? (
              <tr>
                <td colSpan={2} className="px-4 py-10 text-center text-sm text-gray-500">
                  Loading programs / streams...
                </td>
              </tr>
            ) : isError ? (
              <tr>
                <td colSpan={2} className="px-4 py-10 text-center text-sm text-red-600">
                  {getApiErrorMessage(error, "Could not load programs / streams.")}
                </td>
              </tr>
            ) : records.length === 0 ? (
              <tr>
                <td colSpan={2} className="px-4 py-10 text-center text-sm text-gray-500">
                  No programs or streams yet.
                </td>
              </tr>
            ) : (
              records.map((record) => (
                <tr key={record._id} className="hover:bg-gray-50/60">
                  <td className="px-4 py-3 font-medium text-gray-900">{record.streamName}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Button
                        variant="unstyled"
                        onClick={() => onEdit(record)}
                        aria-label={`Edit ${record.streamName}`}
                      ...
                      <Button
                        ...
                        aria-label={`Delete ${record.streamName}`}
      ...
      {pagination && (
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 px-4 py-3 sm:px-6">
          <p className="text-sm text-gray-500">
            {pagination.total} program{pagination.total === 1 ? "" : "s"} / stream{pagination.total === 1 ? "" : "s"}{" "}
            total
