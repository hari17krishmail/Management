...
import { useListVideoCategoriesQuery, type VideoCategoryRecord } from "@/services/video/videoApi";

const PAGE_SIZE = 10;
const SORT_DESCENDING = -1;

type VideoCategoryTableProps = {
  onEdit: (record: VideoCategoryRecord) => void;
  onDelete: (record: VideoCategoryRecord) => void;
};

export function VideoCategoryTable({ onEdit, onDelete }: VideoCategoryTableProps) {
  const [pageNumber, setPageNumber] = useState(1);

  const { data, isLoading, isFetching, isError, error } = useListVideoCategoriesQuery({
    pageNumber,
    pageSize: PAGE_SIZE,
            ...
            {isLoading ? (
              ...
              records.map((record) => (
                <tr key={record._id} className="hover:bg-gray-50/60">
                  <td className="px-4 py-3 font-medium text-gray-900">{record.categoryName}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Button
                        variant="unstyled"
                        onClick={() => onEdit(record)}
                        aria-label={`Edit ${record.categoryName}`}
                      ...
                      <Button
                        ...
                        aria-label={`Delete ${record.categoryName}`}
