...
import { useListFaqCategoriesQuery, type FaqCategoryRecord } from "@/services/faq/faqApi";

const PAGE_SIZE = 10;
const SORT_DESCENDING = -1;

type FaqCategoryTableProps = {
  onEdit: (record: FaqCategoryRecord) => void;
  onDelete: (record: FaqCategoryRecord) => void;
};

export function FaqCategoryTable({ onEdit, onDelete }: FaqCategoryTableProps) {
  const [pageNumber, setPageNumber] = useState(1);

  const { data, isLoading, isFetching, isError, error } = useListFaqCategoriesQuery({
    pageNumber,
    pageSize: PAGE_SIZE,
